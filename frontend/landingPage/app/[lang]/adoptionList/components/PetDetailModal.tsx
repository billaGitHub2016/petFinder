...
const PetDetailModal = (
  ...
  return (
    ...
    >
      <div style={{height: '75vh'}} className="overflow-y-scroll relative pb-16">
        ...
        <div className="flex items-center justify-start mt-4">
          <h3 className="text-lg text-gray-900 mb-0 mr-2">{animalInfo && animalInfo.petName}</h3>
          { animalInfo && animalInfo.petType === 'dog' && <Dog className="mr-2 h-8" />}
          { animalInfo && animalInfo.petType === 'cat' && <Cat className="mr-2 h-8" />}
          { animalInfo && animalInfo.sex === 1 && <Image src="/icons/male.svg" width={20} height={20} alt="公" />}
          { animalInfo && animalInfo.sex === 2 && <Image src="/icons/female.svg" width={20} height={20} alt="母"/>}
      ...
      <div className="absolute bottom-0 left-0 w-full">
          <AdoptButtonGroup></AdoptButtonGroup>
      </div>
    </Modal>
  );
};
...
const AdoptButtonGroup = () => {
  return (
    <div className="flex justify-between items-center w-full pt-1 pb-1 px-6 border-t-1 border-neutral-200 bg-white">
      ...
      <Button type="primary" size="large" shape="round">申请领养资质</Button>
    </div>
  )
}
